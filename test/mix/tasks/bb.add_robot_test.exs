...
defmodule Mix.Tasks.Bb.AddRobotTest do
  use ExUnit.Case
  import Igniter.Test
  ...
  test "creates a robot module with custom name" do
    test_project()
    |> Igniter.compose_task("bb.add_robot", ["--robot", "Test.Robots.MainRobot"])
  ...
  test "adds robot to supervision tree" do
    igniter =
      test_project()
      |> Igniter.compose_task("bb.add_robot", ["--robot", "Test.MyRobot"])
  ...
  test "can add multiple robots" do
    test_project()
    |> Igniter.compose_task("bb.add_robot", ["--robot", "Test.Robot1"])
    |> apply_igniter!()
