...
defmodule BB.ControllerTest do
  use ExUnit.Case, async: true
  alias BB.Dsl.{Controller, Info}
  ...
  describe "controller DSL" do
    defmodule SingleControllerRobot do
      ...
      controllers do
        controller(:path_follower, BB.ControllerTest.TestGenServer)
      end
    ...
    test "controller defined at robot level" do
      ...
      assert controller.child_spec == BB.ControllerTest.TestGenServer
    end
  end
  ...
  describe "controller with options" do
    defmodule ControllerWithOptionsRobot do
      ...
      controllers do
        controller(:velocity_smoother, {BB.ControllerTest.TestGenServer, max_accel: 1.0})
      end
    ...
    test "controller with module and args" do
      ...
      assert controller.child_spec == {BB.ControllerTest.TestGenServer, [max_accel: 1.0]}
    end
  end
  ...
  describe "controllers in supervision tree" do
    defmodule SupervisedControllerRobot do
      ...
      controllers do
        controller(:test_controller, {BB.ControllerTest.TestGenServer, value: 42})
      end
    ...
    test "controller is started and registered" do
      ...
      controller_pid = BBProcess.whereis(SupervisedControllerRobot, :test_controller)
      assert is_pid(controller_pid)
      assert Process.alive?(controller_pid)
    end

    test "controller receives bb context in init" do
      start_supervised!(SupervisedControllerRobot)
      ...
      assert state[:bb] == %{robot: SupervisedControllerRobot, path: [:test_controller]}
    end
  end
    ...
    test "rejects controller with same name as sensor" do
      assert_raise Spark.Error.DslError, ~r/:duplicate/, fn ->
        defmodule ControllerSensorSameName do
          ...
          sensors do
            sensor :duplicate, BB.ControllerTest.TestGenServer
          end

          controllers do
            controller(:duplicate, BB.ControllerTest.TestGenServer)
          end
    ...
    test "rejects controller with same name as link" do
      assert_raise Spark.Error.DslError, ~r/:base_link/, fn ->
        defmodule ControllerLinkSameName do
          ...
          controllers do
            controller(:base_link, BB.ControllerTest.TestGenServer)
          end
