...
defmodule Mix.Tasks.Kinetix.ToUrdf do
  @shortdoc "Export a Kinetix robot definition to URDF format"

  @moduledoc """
  Export a Kinetix robot definition to URDF XML format.

  ## Usage

      mix kinetix.to_urdf MyApp.Robot --output robot.urdf
      mix kinetix.to_urdf MyApp.Robot -o -
  ...
  defp parse_args(args) do
    ...
    cond do
      ...
      rest == [] ->
        {:error, "Usage: mix kinetix.to_urdf MODULE [--output FILE]"}
  ...
  defp export_robot(module, opts) do
    case Exporter.export(module) do
      ...
      {:error, {:not_a_kinetix_module, mod}} ->
        Mix.shell().error("Module #{inspect(mod)} does not use Kinetix (no robot/0 function)")
        exit({:shutdown, 1})
    end
  end
