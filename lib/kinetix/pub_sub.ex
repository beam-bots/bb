...
defmodule Kinetix.PubSub do
  ...
  Subscribers receive messages as:
      ...
      {:kinetix, source_path, %Kinetix.Message{}}
  ...
  Subscribe with `message_types` option to filter by payload type:
      ...
      subscribe(MyRobot, [:sensor], message_types: [Kinetix.Message.Sensor.Imu])
  ...
  ## Examples
      ...
      subscribe(MyRobot, [:sensor, :base_link, :joint1],
        message_types: [Kinetix.Message.Sensor.Imu])
  ...
  ## Examples
      ...
      path = [:sensor | state.kinetix.path]
      publish(state.kinetix.robot, path, message)
  """
  @spec publish(module, [atom], Kinetix.Message.t()) :: :ok
  def publish(robot, path, %Kinetix.Message{} = message)
