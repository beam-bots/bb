...
defmodule Kinetix.Robot.Topology do
  @moduledoc """
  ...
  """
  @spec leaf_links(t(), Kinetix.Robot.t()) :: [atom()]
  def leaf_links(%__MODULE__{link_order: link_order}, robot) do
    Enum.filter(link_order, fn link_name ->
      case Kinetix.Robot.get_link(robot, link_name) do
        %Kinetix.Robot.Link{child_joints: []} -> true
        _ -> false
      end
    end)
  end
