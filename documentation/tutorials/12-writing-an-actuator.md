...
Under `delivery: :pubsub` — `set_position/4`'s default — the command is both published and handed to the actuator by a call, so the caller gets the acceptance or refusal while the publication serves whoever else is watching the topic. `delivery: :broadcast` publishes only, and `delivery: :direct` casts only; neither waits, so neither can report a refusal. The other five commands default to `:broadcast`. Whichever is chosen, the server funnels every transport into `handle_command/2` — your driver never learns which one was used, and choosing one can't skip the checks.
